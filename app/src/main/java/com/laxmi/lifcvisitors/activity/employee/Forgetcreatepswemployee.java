...
import static com.laxmi.lifcvisitors.activity.employee.EmpcreatepswActivity.isValidPassword;
...
//                tv_pattern.setText(getString(R.string.content));
                ...
                if (!newPassword.isEmpty() && !confirmPassword.isEmpty()) {
                    if (!newPassword.equals(confirmPassword)) {
                        Toast.makeText(Forgetcreatepswemployee.this, "Password not matched !!", Toast.LENGTH_SHORT).show();
                    } else {
                        forgotPasswordApi(newPassword);

                    }
                } else {
                    Toast.makeText(Forgetcreatepswemployee.this, "Enter Password !!", Toast.LENGTH_SHORT).show();
            ...
            public void onResponse(@NonNull Call<MSG> call, @NonNull Response<MSG> response) {
                ...
                if (response.body() != null) {
                    ...
                    if (response.body().getMessage().equalsIgnoreCase("Guard Password Update Successfully")) {
                        ...
                        Toast.makeText(Forgetcreatepswemployee.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(Forgetcreatepswemployee.this, "Mobile Number Invalid", Toast.LENGTH_SHORT).show();
